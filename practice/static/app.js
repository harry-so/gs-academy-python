...
function showTime() {
    ...
    api('/api/show_time').then(response => {
        let {year, month , day, hour, minute } = JSON.parse(response);
        stmt = `${year}年${month}月${day}日の${hour}時${minute}分です。`
        console.log(stmt)
        let synthes = new SpeechSynthesisUtterance(stmt);
        synthes.lang = "ja-JP";
        speechSynthesis.speak(synthes);
